...
const ProfileCard = ({ profile }) => {
  ...
  return (
    ...
    >
      <div className={`relative h-full p-4 sm:p-5 md:p-6 bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl border-2 border-gray-800 transition-all duration-300 group-hover:border-${profile.accentColor} group-hover:-translate-y-2 group-hover:shadow-2xl`} style={{ boxShadow: 'inset 0 1px 0 0 rgba(255,255,255,0.05)' }}>
        ...
        <h3 className={`text-lg sm:text-xl font-bold text-center mb-2 sm:mb-3 ${profile.iconColor} transition-all duration-300 group-hover:scale-105`}>{profile.name}</h3>
        <div className="text-center mb-3 sm:mb-4 pb-3 sm:pb-4 border-b border-gray-800/50">
          <div className={`text-2xl sm:text-3xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-${profile.accentColor} to-purple-500 mb-1`}>{profile.stats}</div>
          <div className="text-[10px] sm:text-xs text-gray-500 uppercase tracking-wider font-semibold">{profile.label}</div>
        </div>
        <p className="text-gray-400 text-xs sm:text-sm text-center leading-relaxed mb-3 sm:mb-4">{profile.description}</p>
